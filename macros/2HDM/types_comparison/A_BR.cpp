...
//	Histo with 2HDM results:
	...
	//	Define types of BRs:
		...
		CompareBrs(cosB_As,tanBetas,900,BRs,thdm_fname,type);
	}
...
void DrawGraphs(vector<TGraph>& graphs2HDM, const vector<string> &BRs, const double& tanBeta, const string &type){
	...
	leg.Draw();
	can.Print( (cmsswBase + "/src/Analysis/MssmHbb/macros/pictures/" + type + "_thdm_BRs_cosB_A_mA-900_tanB-" + to_string_with_precision(tanBeta,2) + ".pdf").c_str() );
}
