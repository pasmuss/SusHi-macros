...
def untarSusHiOutput(output_dir):
    ...
    for job in glob.glob(output_dir + 'sushi_out/job_*'):
        ...
        totalnumber = len(glob.glob(output_dir + 'sushi_out/job_*'))
        fractiondone = float(jobnumber)/float(totalnumber)*100
        jobstring = 'job ' + str(jobnumber) + ' of ' + str(totalnumber) + ' untared (' + str(fractiondone) + '%)'
        print jobstring
